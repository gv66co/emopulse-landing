...
function createOrbitRings() {
  const ringMaterial = new THREE.MeshBasicMaterial({ color: 0x94a3b8, transparent: true, opacity: 0.22 });
...
function createCompassNeedle() {
  ...
  const shaftMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.18 });
...
export function updateCompass3D(metrics) {
  ...
  glowMesh.material.uniforms.glowColor.value.set(metrics.stress > 0.6 ? 0xf87171 : metrics.energy > 0.7 ? 0xfbbf24 : 0x3abff8);
  const boost = 0.9 + (metrics.score / 100) * 0.6;
  glowMesh.material.uniforms.intensityBoost.value = boost;
  trajectoryPoints.push(new THREE.Vector3(Math.cos(angle) * 1.6, 0, Math.sin(angle) * 1.6));
